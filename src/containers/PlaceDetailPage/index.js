import { Dialog, Divider, Grid, Stack, styled, Typography } from "@mui/material";
...
function PlaceDetailPage() {
    ...
    // const [currentVideoOpen, setCurrentVideoOpen] = useState(null);
    const [imageDialogOpen, setImageDialogOpen] = useState(false);
    const [videoDialogOpen, setVideoDialogOpen] = useState(false);
    const params = useParams();
    // const [pageData, setPageData] = useState(placeData.find(item => item?.id === params?.id));
    const pageData = placeData.find(item => item?.id === params?.id);
